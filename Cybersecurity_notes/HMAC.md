...
- **a)** Body --> Integrity
- **b)** Http --> Prevents misuse
- **c)** Path/action --> Prevents confusion
- **d)** Timestamp --> Prevents replay
- **e)** Nonce --> Prevents duplication 
- **f)** Sender ID --> Prevents cross-app reuse
...
1. If the first character is wrong → exits immediately
2. If the first 10 characters are correct → runs longer
3. If all characters are correct → runs longest
4. Execution time leaks information
5. An attacker can guess the value one byte at a time.
...
- **a)** Send many requests
- **b)** Measure response time
- **c)** Learn which bytes are correct
- **d)** Forge a valid HMAC
...
- **a)** HMAC tag comparison
- **b)** Password hash comparison
